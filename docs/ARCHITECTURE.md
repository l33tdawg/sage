...
>
> **Phase 1 remnant still in v8.3:** `domain_score` is hardcoded to 0.5 (`app.go:processEpoch`). Real per-domain expertise tracking (per-validator per-domain vote stats + validator domain declaration) is explicit follow-on work (v8.4+) — the math in `poe.ComputeWeight` already weights it, but the input comes from a cold-start prior until that source lands.

...
| `brain.explore-memory` | brain | Render Memory MRI, filter domain/status, orbit/zoom/select, inspect loaded memory, switch to the synchronized table, preserve focus across invalidation notices, and fall back to that same table with explicit retry when renderer initialization fails | `BrainView`, `BrainViewModel`, `MetalBrainView`; memory graph API and SSE | `native-control` | Hosted retry-action acceptance, MRI pacing, large-store/offline and deeper behavioral/accessibility evidence | implemented; acceptance open |

<!-- Reference index reconciled for SAGE v11.5.0. Core REST, MCP, concepts, Python SDK, federation/brain graph, reranker, and environment references are current-facing for v11. -->
...
| [`federation-and-brain-api.md`](federation-and-brain-api.md) | The v11 HTTP surface: cross-network federation (`/fed/v1/*` mTLS listener, `/v1/federation/*` operator REST, `/v1/dashboard/federation/*` proxy), the v11.5 domain-sync replication surface (`/fed/v1/sync/*` push + anti-entropy digest, `sync_domains` consent CRUD + status), and the memory train-of-thought endpoint (`GET /v1/dashboard/memory/{id}/related`). All off-consensus; only tx-33/34 reach chain state directly (sync copies arrive as ordinary locally-signed tx-1 submits). |

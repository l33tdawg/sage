...
| `SessionEnd` | `sage-session-end.sh` | **direct-write** | Calls `sage-gui hook session-end`, which submits a `session-lifecycle` observation memory through the node's consensus path — a BFT quorum on a multi-validator network, one signed validator on a personal install — so the timeline shows session bookends. Soft-fails silently if SAGE isn't reachable — never blocks the agent's exit path. |
...
| `UserPromptSubmit` | `sage-user-prompt.sh` | passive pointer + nudge | In every memory mode, calls `sage-gui hook inbox-status` to surface payload-free exact-agent coordination without claiming work. The existing unclaimed count and lease-free work-wake snapshot still expose unfinished work. A separate activity snapshot adds a one-shot cue when assignments or passive replies change. `full` also reminds the agent to call `sage_turn`; `bookend` and `on-demand` suppress only that memory nudge. Ordinary probe failures are reported as unavailable, never as zero. |
...
4. Session boundary operations soft-fail through their safe wrapper behavior. The established inbox count/work-wake probes remain fail-visible: the wrapper says the check is unavailable and never manufactures a zero count. The additive activity route is backward-compatible with nodes that return `404` or `501` and never hides an ordinary inbox pointer.

`UserPromptSubmit` reads `GET /v1/inbox/activity-state` only after the existing inbox count and v1 work-wake checks succeed. Its exact `{version:1,epoch,seq}` snapshot is presentation state, not unfinished-work state: it never changes `Stop`, claims work, or exposes assignment/reply payloads. The opaque 32-character hexadecimal `epoch` identifies the database incarnation; it survives process restart and backup restore but changes with a fresh database, preventing an older high cursor from suppressing new activity after reinitialization. The host stores one epoch-fenced monotonic cursor in a private, bounded namespace keyed by the exact hook agent, provider, and canonical project/workspace—not by an ephemeral session ID. Competing hook processes serialize on that scope. The hook emits the cue before persisting the cursor, so an output or storage failure can cause a duplicate later but cannot silently lose the signal. Fresh cursors start at zero; the server does not backfill historical activity. Only `404` and `501` preserve backward-compatible silence; other activity-probe failures remain fail-visible through the hook wrapper.
...
- **`on-demand`** — automatic memory calls are skipped entirely; the agent drives `sage_recall` / `sage_reflect` explicitly. Payload-free coordination polling on UserPromptSubmit remains enabled because memory cadence does not control inbox visibility.
...
SAGE ships **two SessionStart/SessionEnd direct-write hooks**, payload-free exact-agent work and activity pointers on `UserPromptSubmit`, and **nudge hooks** where direct-write would be too noisy (`UserPromptSubmit`, `PreCompact`) or too high-frequency without batching (`SubagentStop`). The inbox pointers never claim or expose messages. Activity is not work: only the existing v1 work-wake sequence can influence the `Stop` hook. Conversation-level memory remains the agent's job (via `sage_turn`, `sage_reflect`) since only the LLM has enough context to distill what's worth remembering. The `memory_mode` flag tunes memory nudges independently from coordination visibility.

...
The connection policy is local-first: Codex uses the local SAGE MCP server. SAGE does not install or autostart Cloudflare as part of its default setup. Remote MCP is an explicit advanced path over a LAN/VPN bind or a trusted HTTPS endpoint the operator manages; ChatGPT Work's OpenAI Secure MCP Tunnel remains a separate compatibility path.

...
const stateSyncBootstrapCompleteValueSize = 8 + 1 + 8 + sha256.Size

var (
	stateSyncBootstrapCompleteKey   = []byte("SAGE:STATE_SYNC_BOOTSTRAP_COMPLETE")
	stateSyncBootstrapCompleteMagic = []byte("SAGE-SSB")
)
...
// SaveSeenCommit saves and synchronously flushes a seen commit, used by e.g.
// the state sync reactor when bootstrapping node. State sync publishes the
// positive-height state only after this returns, so restart can always
// reconstruct LastCommit once StateStore.Bootstrap becomes visible.
func (bs *BlockStore) SaveSeenCommit(height int64, seenCommit *types.Commit) error {
	...
	return bs.db.SetSync(calcSeenCommitKey(height), seenCommitBytes)
}

// SaveStateSyncBootstrapComplete synchronously records that the state-sync
// bootstrap has persisted its commit and state and successfully handed the
// synchronized state to the block-sync reactor. SAGE keeps ordinary ABCI
// serving closed until this exact height/AppHash record is visible.
func (bs *BlockStore) SaveStateSyncBootstrapComplete(height int64, appHash []byte) error {
	if height <= 0 {
		return errors.New("state-sync bootstrap completion height must be positive")
	}
	if len(appHash) != sha256.Size {
		return fmt.Errorf("state-sync bootstrap completion AppHash must be %d bytes", sha256.Size)
	}
	encoded := make([]byte, stateSyncBootstrapCompleteValueSize)
	copy(encoded, stateSyncBootstrapCompleteMagic)
	encoded[len(stateSyncBootstrapCompleteMagic)] = 1
	binary.BigEndian.PutUint64(encoded[len(stateSyncBootstrapCompleteMagic)+1:], uint64(height))
	copy(encoded[len(stateSyncBootstrapCompleteMagic)+1+8:], appHash)
	return bs.db.SetSync(stateSyncBootstrapCompleteKey, encoded)
}

// LoadStateSyncBootstrapComplete returns the durable block-sync handoff record.
// A missing record is represented by height zero and a nil AppHash. Malformed
// records return an error rather than being treated as persistence lag.
func (bs *BlockStore) LoadStateSyncBootstrapComplete() (int64, []byte, error) {
	encoded, err := bs.db.Get(stateSyncBootstrapCompleteKey)
	if err != nil {
		return 0, nil, fmt.Errorf("load state-sync bootstrap completion: %w", err)
	}
	if len(encoded) == 0 {
		return 0, nil, nil
	}
	if len(encoded) != stateSyncBootstrapCompleteValueSize ||
		!bytes.Equal(encoded[:len(stateSyncBootstrapCompleteMagic)], stateSyncBootstrapCompleteMagic) ||
		encoded[len(stateSyncBootstrapCompleteMagic)] != 1 {
		return 0, nil, errors.New("state-sync bootstrap completion record is malformed")
	}
	height := binary.BigEndian.Uint64(encoded[len(stateSyncBootstrapCompleteMagic)+1:])
	if height == 0 || height > uint64(^uint64(0)>>1) {
		return 0, nil, errors.New("state-sync bootstrap completion height is invalid")
	}
	hashOffset := len(stateSyncBootstrapCompleteMagic) + 1 + 8
	return int64(height), append([]byte(nil), encoded[hashOffset:]...), nil
}

// RecoverIncompleteStateSyncBootstrap removes the one exact block-store
// residue possible when a process crashes after synchronously persisting the
// state-sync seen commit but before StateStore.Bootstrap publishes any state.
// The caller must first prove the Comet state DB is canonically empty. Any
// additional, malformed, or ordinary block-store data is preserved so the
// receiver freshness gate can reject it.
func (bs *BlockStore) RecoverIncompleteStateSyncBootstrap() (bool, error) {
	bs.mtx.Lock()
	defer bs.mtx.Unlock()
	if bs.base != 0 || bs.height != 0 {
		return false, nil
	}
	height, recovered, err := recoverIncompleteStateSyncBootstrap(bs.db)
	if recovered {
...
// RecoverIncompleteStateSyncBootstrapDB is the startup-safe form of exact
// commit-only recovery. It deliberately inspects the raw database before a
// BlockStore is constructed because upstream NewBlockStore panics while
// decoding malformed blockStore metadata. Corrupt or ambiguous input remains
// untouched for the caller's freshness gate to reject.
func RecoverIncompleteStateSyncBootstrapDB(db dbm.DB) (bool, error) {
	_, recovered, err := recoverIncompleteStateSyncBootstrap(db)
	return recovered, err
}
...
func recoverIncompleteStateSyncBootstrap(db dbm.DB) (int64, bool, error) {
	...
	if err != nil {
		return 0, false, fmt.Errorf("inspect incomplete state-sync bootstrap: %w", err)
	}
	if !iterator.Valid() {
		iteratorErr := iterator.Error()
		closeErr := iterator.Close()
		if iteratorErr != nil {
			return 0, false, fmt.Errorf("inspect incomplete state-sync bootstrap: %w", iteratorErr)
		}
		if closeErr != nil {
			return 0, false, fmt.Errorf("close incomplete state-sync bootstrap iterator: %w", closeErr)
		}
		return 0, false, nil
	}
	...
	if hasAdditionalData || !bytes.HasPrefix(key, []byte("SC:")) {
		return 0, false, nil
	}
	height, err := strconv.ParseInt(string(key[len("SC:"):]), 10, 64)
	if err != nil || height <= 0 || !bytes.Equal(key, calcSeenCommitKey(height)) {
		return 0, false, nil
	}
	protoCommit := new(cmtproto.Commit)
	if err := proto.Unmarshal(value, protoCommit); err != nil {
		return 0, false, nil
	}

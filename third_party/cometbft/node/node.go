...
func NewNodeWithContext(ctx context.Context,
	...
	offlineStateSyncHeight, err := loadOfflineStateSyncHeight(blockStore.Height(), blockExec.Store())
	if err != nil {
	...
	// Keep the effective height durable while the block store is empty. Clearing
	// it during reactor construction creates a second-restart crash window: a
	// partitioned node can stop before H+1 is stored, then restart with state H,
	// block store 0, and no effective-height bridge. Once the block store has a
	// block this marker is ignored, so retaining it is both safe and idempotent.
...
type offlineStateSyncHeightReader interface {
	GetOfflineStateSyncHeight() (int64, error)
}

func loadOfflineStateSyncHeight(blockStoreHeight int64, stateStore offlineStateSyncHeightReader) (int64, error) {
	if blockStoreHeight != 0 {
		return 0, nil
	}
	height, err := stateStore.GetOfflineStateSyncHeight()

...
func startStateSync(
	...
	go func() {
		...
		// Persist the commit before publishing the bootstrapped state. Consensus
		// reconstructs LastCommit from the block store as soon as the state store
		// reports a positive height. Publishing state first leaves a crash window
		// where restart panics because neither a seen nor block commit exists.
		err = completeStateSyncBootstrap(stateStore, blockStore, bcR, state, commit)
		if err != nil {
			ssR.Logger.Error("Failed to complete state-sync bootstrap", "err", err)
			return
		}
	}()
	return nil
...
type stateSyncBootstrapStateStore interface {
	Bootstrap(sm.State) error
}

type stateSyncBootstrapCommitStore interface {
	SaveSeenCommit(int64, *types.Commit) error
	SaveStateSyncBootstrapComplete(int64, []byte) error
}

func persistStateSyncBootstrap(
	stateStore stateSyncBootstrapStateStore,
	commitStore stateSyncBootstrapCommitStore,
	state sm.State,
	commit *types.Commit,
) error {
	if stateStore == nil || commitStore == nil || commit == nil {
		return errors.New("state-sync bootstrap requires state, commit, and stores")
	}
	if err := commitStore.SaveSeenCommit(state.LastBlockHeight, commit); err != nil {
		return fmt.Errorf("store state-sync seen commit: %w", err)
	}
	if err := stateStore.Bootstrap(state); err != nil {
		return fmt.Errorf("publish state-sync state: %w", err)
	}
	return nil
}

func completeStateSyncBootstrap(
	stateStore stateSyncBootstrapStateStore,
	bootstrapStore stateSyncBootstrapCommitStore,
	blockSync blockSyncReactor,
	state sm.State,
	commit *types.Commit,
) error {
	if blockSync == nil {
		return errors.New("state-sync bootstrap requires a block-sync reactor")
	}
	if err := persistStateSyncBootstrap(stateStore, bootstrapStore, state, commit); err != nil {
		return err
	}
	if err := blockSync.SwitchToBlockSync(state); err != nil {
		return fmt.Errorf("switch to block sync: %w", err)
	}
	if err := bootstrapStore.SaveStateSyncBootstrapComplete(state.LastBlockHeight, state.AppHash); err != nil {
		return fmt.Errorf("persist state-sync block-sync handoff: %w", err)
	}
	return nil
}

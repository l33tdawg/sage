...
// rejectPeerIfSyncing rejects a peer only while a state sync session is active.
// Receive can process invalid snapshot advertisements while syncer is nil, and
// Sync installs and clears syncer concurrently with peer traffic.
func (r *Reactor) rejectPeerIfSyncing(peer p2p.Peer) {
	r.mtx.RLock()
	defer r.mtx.RUnlock()

	if r.syncer != nil {
		r.syncer.RejectPeer(peer)
	}
}

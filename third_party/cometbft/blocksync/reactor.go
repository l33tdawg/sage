...
// ErrStateSyncSealAborted is returned by SAGE's boot-only application gate
// when block sync reached the application before the post-state-sync durable
// seal could complete. The node is already stopping; treating this one error
// as a graceful reactor abort avoids converting a fail-closed startup error
// into an unconditional process panic.
var ErrStateSyncSealAborted = errors.New("state-sync application seal aborted")

func isStateSyncSealAbort(err error) bool {
	return errors.Is(err, ErrStateSyncSealAborted)
}
...
FOR_LOOP:
	for {
		...
		case <-didProcessCh:
			...
			if err != nil {
				if isStateSyncSealAbort(err) {
					r.Logger.Error("State-sync application seal aborted block sync",
						"height", first.Height,
						"err", err,
					)
					break FOR_LOOP

...
func (store dbStore) Bootstrap(state State) error {
	...
	// Bootstrap publishes a state whose height can be ahead of an empty block
	// store. Persist the effective block-sync height in the same synchronous
	// batch so every crash boundary—including immediately after Bootstrap—can
	// restart without the state/store mismatch panic. NewNode keeps the marker
	// while the block store is empty and ignores it once a real block-store
	// height supersedes the bridge.
	if state.LastBlockHeight > 0 {
		if err := batch.Set(offlineStateSyncHeight, int64ToBytes(state.LastBlockHeight)); err != nil {
			return err
		}
	}

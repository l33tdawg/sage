...
func TestStateSyncAuthorizationRejectsIncompleteJoinBindings(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "version below supported set", mutate: func(c *JoinAuthorizationConfig) { c.AppVersion = 19 }, want: "version (20, 21, 22, 23, 24, 25, or 26)"},
		{name: "version above supported set", mutate: func(c *JoinAuthorizationConfig) { c.AppVersion = 27 }, want: "version (20, 21, 22, 23, 24, 25, or 26)"},
...
func TestStateSyncAuthorizationSupportsV20ThroughV26Exactly(t *testing.T) {
	now := time.Unix(1_700_000_000, 0).UTC()
	for _, version := range []uint64{20, 21, 22, 23, 24, 25, 26} {
	...
	assert.False(t, SupportsAppVersion(27))
}
